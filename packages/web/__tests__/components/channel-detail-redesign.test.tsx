...
describe('AgentsContentChannelDetail redesign', () => {
  ...
  it('renders a connected Feishu channel with status bar, conversation, and activity', async () => {
    ...
    expect(host.textContent).toContain('In Feishu console, enable Bot and subscribe to im.message.receive_v1.');
    ...
    expect(host.textContent).not.toContain('Verification Token');
    expect(host.textContent).not.toContain('Webhook URL');
    expect(host.textContent).not.toContain('Transport');
    expect(host.textContent).not.toContain('Long Connection');
    expect(host.textContent).not.toContain('Waiting for verification');
    expect(host.textContent).not.toContain('Feishu authorization');
    expect(host.textContent).not.toContain('Authorize with Feishu');
  ...
  it('shows optional Feishu user binding when credentials exist but verification is disconnected', async () => {
    ...
    expect(host.textContent).toContain('Optional user binding');
    expect(host.textContent).toContain('Bind Feishu user');
    expect(host.textContent).not.toContain('QR one-click app creation');
    expect(host.textContent).not.toContain('not wired into this local flow yet');

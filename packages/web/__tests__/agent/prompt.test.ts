...
describe('agent prompt self-introduction rules', () => {
  ...
  it('uses the same prompt for chat and agent surfaces', () => {
    ...
    expect(CHAT_SYSTEM_PROMPT).toContain('If the user\'s message already contains a concrete task');
    expect(CHAT_SYSTEM_PROMPT).toContain('skip the self-introduction and do the task directly');
  });
});

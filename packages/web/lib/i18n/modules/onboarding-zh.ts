...
export const onboardingZh = {
  ...
  setup: {
    ...
    agentCliModeTitle: 'CLI 模式已就绪',
    agentCliModeDesc: '支持执行 shell 命令的 Agent 可以直接调用 mindos CLI，无需安装 MCP 配置。需要自动工具发现时，再打开 MCP。',
    agentTransport: '传输方式',
    agentScope: '范围',
    ...
    healthAgentsCliReady: 'CLI 模式已就绪，支持 shell 的 Agent 可直接使用 MindOS 命令。',
    healthAgentsCliMcpOptional: 'CLI 模式已就绪。MCP Agent 可稍后在 设置 → 连接 中添加。',

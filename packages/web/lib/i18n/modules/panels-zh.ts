...
export const panelsZh = {
  ...
  agentsContent: {
    ...
    agentSubtitle: '管理本地对话 runtime、本机 Agent 客户端、ACP 编程 Agent 与 A2A 远程 Agent。',
    runtime: {
      title: '执行端点',
      description: '选择下一段对话由哪个本地 runtime 执行：MindOS 自身，或用户本机安装的编程 Agent。',
      panelTitle: '执行端点',
      panelDescription: '用 MindOS、Codex 或 Claude Code 打开对话。',
      refresh: '刷新运行状态',
      ...
      mindosDescription: '内置助手，面向本地知识库、文件、工具与已保存对话。',
      mindosDetail: '可直接通过 MindOS Chat Panel 使用，并沿用你选择的 provider 和模型。',
      codexDescription: '使用这台机器上的 Codex 安装，承接本地编码会话与 Codex thread 历史。',
      claudeDescription: '使用这台机器上的 Claude Code 安装，承接本地编码会话与 Claude session 历史。',
      nativeAvailableDetail: '已在本机检测到，MindOS 可以使用。',
      checkingDetail: (name: string) => `正在检测本地 ${name} 状态...`,
      missingDetail: (name: string) => `${name} 暂时还不能被 MindOS server 进程使用。`,
    ...
    localClients: {
      title: '本机 Agent 客户端',
      panelTitle: '本机 Agent 客户端',
      description: '这台机器上检测到的 Agent App 与 CLI。这里表示客户端和配置就绪状态，和上面的当前对话 runtime 分开。',
      statusConnected: '已配置',
      statusDetected: '已检测',
      statusNotFound: '未找到',
    ...
    acpAgents: {
      title: 'ACP 编程 Agent',
      description: 'MindOS 可通过 ACP 直接调用的协议原生编程 Agent，包含安装状态、快捷动作与命令覆盖配置。',
    },
    a2aAgents: {
      title: 'A2A 远程 Agent',
      description: '通过 A2A 发现的远程 Agent，用于跨 Agent 委派；它和本地对话 runtime、ACP 编程 Agent 分开管理。',
    },
    ...
    presets: {
      title: 'Assistant',
      subtitle: '管理 MindOS 的 Assistant 档案及其产品入口。',
      activeLabel: '已启用',
      draftLabel: '草稿',
      plannedLabel: '计划中',
      presetRail: 'Assistant 档案',
      promptTitle: '默认 Prompt',
      promptHint: '已启用助手会写入自己的 .mindos Prompt 文件；草稿助手暂存在本机。',
      toolsTitle: '工具',
      skillsTitle: '技能',
      contextTitle: '上下文',
      triggersTitle: '运行入口',

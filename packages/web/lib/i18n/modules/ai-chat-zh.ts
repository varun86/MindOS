...
export const aiChatZh = {
  ask: {
    ...
    saveDirectly: '直接保存',
    saveDirectlyDesc: '保存原文',
    organizeToNote: '整理为笔记',
    organizeToNoteDesc: '提炼重点',
    targetFolder: '文件夹',
    fileName: '文件名',
    ...
    sessionContext: {
      ...
      addAssistant: '添加助理',
      searchSpaces: '搜索空间',
      searchAssistants: '搜索助理',
      noMatches: '没有匹配项',
      newSession: '新会话',
      removeItem: (label: string) => `移除 ${label}`,
      spacePlaceholder: '空间路径',
      assistantPlaceholder: '助理 ID',
      applyNextTurn: '变更从下一条消息生效。',
      spacesCount: (n: number) => `${n} 个空间`,
      assistantsCount: (n: number) => `${n} 个助理`,
    },
  },
  changes: {
    unreadBanner: (n: number) => `${n} 条内容变更未读`,
    agentUnreadBanner: (n: number, files: number) => `${n} 条 Agent 改动待审阅${files > 0 ? `，涉及 ${files} 个文件` : ''}`,
    activityNoticeTitle: (n: number) => `${n} 条新变更`,
    activityNoticeMeta: '查看最近编辑记录。',
    agentReviewNoticeTitle: (files: number) => files > 0
      ? `Agent 更新了 ${files} 个文件`
      : 'Agent 做了改动',
    agentReviewNoticeMeta: (n: number) => `${n} 处改动待你确认`,
    reviewNow: '立即查看',

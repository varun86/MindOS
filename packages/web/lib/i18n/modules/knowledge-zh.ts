...
export const knowledgeZh = {
  home: {
    ...
    newNote: '新建笔记',
    mindTagline: '本地 Markdown 上的个人心智工作台。',
    builtInSpacesTitle: '心智系统',
    builtInSpacesDesc: '',
    ...
    mindAssistant: {
      ...
      openInstruction: '说明',
      openDrafts: '草稿',
      moreAssistants: (n: number) => `+${n} 个助理`,
      assistantCount: (n: number) => `${n} 个助理`,
      viewAllAssistants: (n: number) => `查看全部 ${n} 个助理`,
      ...
      runPrompt: (spaceTitle: string, assistantName: string, assistantDesc: string, spacePath: string, promptPath: string) => `请作为 MindOS 空间「${spaceTitle}」（${spacePath}）里的「${assistantName}」助理工作。
...
请先读取助理 Prompt：${promptPath}，再读取这个空间的 INSTRUCTION.md，并查看同一空间内相关笔记。请产出一份简洁的 Markdown 草稿供用户确认，不要直接覆盖正式笔记。如果需要写入文件，请写到 ${spaceDraftsPath(spacePath)} 下，并使用带日期的文件名。草稿要聚焦、可追溯，并方便用户确认或丢弃。`,
  ...
  inbox: {
    ...
    organizationAgentTitle: '收集箱整理助理',
    organizationAgentPreviewDesc: '进入审核后选择材料，整理助理会先给整理建议，写入前由你确认。',
    organizationAgentDesc: '选择下方材料，整理助理会先给整理建议，写入前由你确认。',
    organizationAgentReady: (n: number) => `已选择 ${n} 条，可以开始整理。`,
    organizationCommandTitle: '整理到心智',
    organizationAssistantInitial: '整',
    organizationAssistantBadge: '内置助理',
    ...
    organizationAssistantWritesMind: '确认后的心智更新',
    ...
    doneEmptyDesc: '完成的整理到心智记录会出现在这里。',
    ...
    organizeNoAi: '请先配置 AI API 密钥再运行收集箱整理助理。没有 AI 时仍可正常保存内容。',
    organizeBusy: '收集箱整理助理已经在运行。',
    organizeReadFailed: (failed: number, skipped: number) =>
      `收集箱整理助理已开始处理可读取文件。${failed} 个读取失败，${skipped} 个格式不支持已跳过。`,
    ...
    viewOrganizeTip: '运行收集箱整理助理，将这些文件分类到知识库的合适位置。',

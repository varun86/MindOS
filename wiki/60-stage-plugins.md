<!-- Last verified: 2026-06-22 | Current version: v1.1.46 -->
...
- **Installed**：管理 MindOS 内置渲染器，并查看已导入的 Obsidian 插件运行状态、包位置、能力覆盖和静态 surface summary；legacy `<mindRoot>/.plugins/<id>` 包可从这里显式迁移到 canonical `<mindRoot>/.mindos/plugins/<id>`。
...
- **Import**：扫描本机 Obsidian vault，按兼容性选择插件复制到当前 MindOS `<mindRoot>/.mindos/plugins/`；默认读取 `<vault>/.obsidian/plugins/`，也支持用户指定 Obsidian config folder 以读取 `<vault>/<obsidianConfigFolder>/plugins/`。
...
Obsidian editor command 现在有 Markdown active-file limited context：Command Center、hotkey host 和 Installed host 在 `/view/*.md` 下执行 command 时会传入 `editorContext.sourcePath`，后端用 Markdown editor facade 执行 `editorCallback` / `editorCheckCallback` 并把变更写回当前文件；无 active Markdown 文件时仍保持 catalog/recorded，不承诺完整 CodeMirror host。
...
Obsidian settings host 现在分两层：legacy `PluginSettingTab.display()` / `Setting` controls 可以通过 `/api/obsidian-plugins/settings` 捕获并在 Installed host 中编辑；Obsidian 1.13+ 的 `getSettingDefinitions()` 会被同一路由序列化为 declarative settings，展示 group/list/page/control/action/render、control key/value/options 和 unsupported signals。基础 declarative controls 已进入 limited editable host，走类型校验、per-control `validate` 与 `setControlValue` 持久化；静态 enabled action 进入 explicit-confirmation limited host，必须先经 UI/API 确认才会执行；list add/delete/reorder 也进入 explicit-confirmation limited host，带索引边界校验和 callback 失败时的插件 settings/data rollback；custom page/render 可显式确认后运行 snapshot preview，只返回安全 text/node/pageItems 摘要并恢复插件 settings/data。full custom DOM page/render host、dynamic disabled action/control 仍保持 future gate；list/preview rollback 不覆盖 callback 已写出的任意 vault/file/native 副作用。
真实 Obsidian 社区插件 smoke 矩阵现在覆盖 16 个代表插件：Style Settings、QuickAdd、Tag Wrangler、Homepage、Minimal Theme Settings、Linter、Calendar (Beta)、Admonition 必须在当前受限 runtime load；Dataview / Tasks / Kanban / Iconize / Advanced Tables / Outliner / Editing Toolbar 作为 blocked/skipped future-gate 样本，避免把复杂生态误报成已兼容。
轻量 runtime shim 已覆盖 `Vault.getConfig/setConfig`、sidebar leaf、`moment.locale/localeData`、document class/query lookup、`setCssProps/setCssStyles`、`EditorSuggest/Scope` catalog、`app.commands` facade、`getIconIds`、`app.customCss` facade、CodeMirror mode / CodeMirrorAdapter no-op map；这些能力只服务受限 runtime / snapshot/catalog host，不代表 full DOM、完整 workspace pane 或 CodeMirror host 已开放。
...
Settings > Plugins 的顶部摘要、Installed 计数和 MindOS renderer 行里的 surface 数量读取 `/api/plugins/catalog`；Obsidian plugin host 的启用、加载、设置、卸载和 legacy package migration 等生命周期操作仍走 `/api/obsidian-plugins`。
...
远程更新也是显式确认 API：只针对已有社区来源且已位于 canonical `.mindos/plugins/<id>` 的插件，把包文件替换到当前 MindOS root 的 canonical 包目录；若该插件仍只存在于 legacy `.plugins/<id>`，必须先在 Installed host 走显式迁移流程，再执行 community update。更新保留 `data.json`、本机额外普通文件/目录和 enabled state，不保留 symlink / 特殊文件，也不从网页端接受 `targetMindRoot`。若插件在当前 runtime 已加载，更新前只卸载旧 runtime 并刷新 discover；新包不会自动 load，用户仍可在 Installed host 中手动管理。

...
### 自定义 hook 不要在 render 阶段读写 ref.current（2026-05-10）

**症状**：React compiler lint 报 `react-hooks/refs`，典型位置是自定义 hook 为了避免事件回调 stale closure，在组件 render 阶段直接执行 `someRef.current = value`，或用 `someRef.current` 初始化 state。
